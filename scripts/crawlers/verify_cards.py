#!/usr/bin/env python3
"""
All-banks card data verification crawler.
Compares CSV fields to what we can extract from each card's application_link page.
Single output: verification_report.csv (all issuers).
"""
import os
import re
import time
...
# Paths: script lives in scripts/crawlers/
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.join(SCRIPT_DIR, "..", "..")
CSV_PATH = os.path.join(PROJECT_ROOT, "public", "cards.csv")
REPORT_PATH = os.path.join(SCRIPT_DIR, "verification_report.csv")
...
REPORT_COLUMNS = ["issuer", "card_name", "field", "csv_value", "website_value", "status", "source_url"]
...
def run():
    ...
    for i, row in enumerate(all_rows):
        ...
        try:
            ...
            time.sleep(1)  # polite crawling (increase to 2 if you get rate-limited)
