...
export default function ResultsPage() {
  ...
  // ---------------------
  // Live Re-Scoring (Primary Type) with HARD brand filter
  // ---------------------
  useEffect(() => {
    ...
    let selected: { card: Card; score: number }[] = scoredRaw;
    ...
    const wantsHotelBrand =
      ...
      if (wantsAirlineBrand || wantsHotelBrand) {
        ...
        const brandCards = scoredRaw.filter(x =>
          (x.card.card_family || "").toLowerCase().includes(brandKey)
        );
        const nonBrandCards = scoredRaw.filter(
          x => !(x.card.card_family || "").toLowerCase().includes(brandKey)
        );
  
        const topBrand = [...brandCards]
          .sort((a, b) => b.score - a.score)
          .slice(0, 2);
  
        const rest = dedupeByFamily(nonBrandCards).sort(
          (a, b) => b.score - a.score
        );
  
        selected = [...topBrand, ...rest];
      } else {
        selected = dedupeByFamily(scoredRaw);
      }
  

    const finalCards = selected
      .sort((a, b) => b.score - a.score)
      .map(x => x.card);
