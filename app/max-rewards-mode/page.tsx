...
export default function MaxRewardsModePage() {
  ...
  return (
    <div className="max-rewards-page min-h-screen bg-[var(--surface)] text-[var(--text-primary)]" data-card-mode="personal">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:flex-wrap sm:items-end">
            <label className="flex flex-col gap-1">
              <span className="text-sm font-semibold text-[var(--text-secondary)]">
                To maximize rewards, how much are you willing to spend? <span style={{ color: theme.primary }}>*</span>
              </span>
              <input
                type="text"
                inputMode="numeric"
                  ...
                  >
                    <CardTile
                      ...
                      onOwnedChange={(cardName, haveIt) =>
                        setOwnedCards((prev) => (haveIt ? [...prev, cardName] : prev.filter((n) => n !== cardName)))
                      }
                    />
                  </div>
                );
