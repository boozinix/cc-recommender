/**
 * Compare Chase crawler output (crawled/chase.json) to Chase cards in public/cards.csv.
 * Run after crawl:chase. Usage: npm run crawl:chase:compare  or  npx tsx crawlers/compareChaseToCsv.ts
 */
...
function loadCsvChaseCards(): CsvChaseCard[] {
  const csvPath = path.join(process.cwd(), "public", "cards.csv");
  const raw = fs.readFileSync(csvPath, "utf-8");
  const parsed = Papa.parse<Record<string, string>>(raw, { header: true, skipEmptyLines: true });
  return parsed.data
    .filter((row) => (row.issuer || "").trim().toLowerCase() === "chase")
    .map((row) => ({
...
function loadCrawledChase(): CardCrawlerResult[] {
  const jsonPath = path.join(process.cwd(), "crawled", "chase.json");
  if (!fs.existsSync(jsonPath)) {
    throw new Error("crawled/chase.json not found. Run npm run crawl:chase first.");
  }
  const raw = fs.readFileSync(jsonPath, "utf-8");
  return JSON.parse(raw) as CardCrawlerResult[];
}

function findBestCrawlerMatch(csvCard: CsvChaseCard, crawled: CardCrawlerResult[]): CardCrawlerResult | undefined {
...
interface ComparisonRow {
  csvCard: CsvChaseCard | null;
  crawled?: CardCrawlerResult;
  match: MatchType;
  diffs?: { kind: DiffKind; csv: string; crawled: string }[];
}

function compare(csvCards: CsvChaseCard[], crawled: CardCrawlerResult[]): ComparisonRow[] {
...
function printReport(rows: ComparisonRow[], crawled: CardCrawlerResult[]): void {
  ...
  console.log("\n=== Chase: Crawler vs cards.csv ===\n");
  console.log(`Compared ${rows.length} rows (CSV Chase cards + crawler-only).\n`);
...
function main(): void {
  const csvCards = loadCsvChaseCards();
  const crawled = loadCrawledChase();
  console.log(`Loaded ${csvCards.length} Chase cards from public/cards.csv`);
  console.log(`Loaded ${crawled.length} cards from crawled/chase.json`);
