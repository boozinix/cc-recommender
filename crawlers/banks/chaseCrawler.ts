/**
 * Chase credit card crawler.
 * Fetches card pages from creditcards.chase.com and extracts bonus, spend, and fee.
 * URLs are loaded from public/cards.csv (application_link for issuer=Chase).
 */
...
function extractBonusText($: ReturnType<typeof loadHtml>): string {
  ...
  // "Earn a $200 bonus after you spend $500..."
  const earnMatch = body.match(/Earn\s+(?:a\s+)?(?:\$?\d[^.]*?)(?:after[^.]*?\.|\.)/i);
  if (earnMatch) return earnMatch[0].trim();
  // "$200 bonus" or "60,000 points" in hero/headline
  const bonusMatch = body.match(/(\$?\d{1,3}(?:,\d{3})*(?:,\d{3})*)\s*(?:bonus|points?|miles?)/i);
...
function extractFeeText($: ReturnType<typeof loadHtml>): string {
  const body = $("body").text();
  // Chase uses multiple formats: "$95 annual fee", "ANNUAL FEE $95", "annual fee: $95", "$95/year"
  const patterns: RegExp[] = [
    /\$(\d+)\s*annual fee/i,           // $95 annual fee
    /annual fee\s*[:\s]*\$?(\d+)/i,   // annual fee $95 or annual fee: 95
    /ANNUAL FEE\s*\$?(\d+)/i,         // ANNUAL FEE $95 (section header)
    /annual fee\s+of\s+\$?(\d+)/i,    // annual fee of $95
    /\$(\d+)\s*\/\s*year/i,           // $95/year (less common)
  ];
  ...
  const noFee = body.match(/(?:No annual fee|\$0\s*annual fee|you won't have to pay an annual fee)/i);
...
function buildResult(
  ...
  // Reject extracted values that look wrong: years, unreasonable amounts
  ...
  if (bonus.currency === "dollars" && bonus.amount != null && validBonus(bonus.amount)) {
    ...
    estimatedBonusValueUsd = signupBonusAmount; // rough fallback for points (1 cpp)
  }
...
export async function fetchChaseCards(): Promise<CardCrawlerResult[]> {
  const csvCards = getChaseCardsFromCsv();
  logInfo("Chase cards from CSV", { count: csvCards.length });

  // Group cards by URL so we fetch each URL once but emit one result per CSV card
  const urlToCsvCards = new Map<string, ChaseCsvCard[]>();
  ...
  for (const [url, csvCardsForUrl] of urlToCsvCards) {
    ...
    if (linkCheck.status !== "ok") {
      ...
      const search = await searchForCardLink(firstCard.card_name, "Chase", url);
      linkOpts = {
        ...linkOpts,
